...
fn main() {
    ...
    let mut ir =
        Ir::from_yaml_str(input1, "run-screenshots.yaml").expect("can deserialze 1 from yaml");
    let ir2 = Ir::from_yaml_str(input2, "global-config.yaml").expect("can deserialze 2 from yaml");
